...
Texture2D	real_tex	: register(t1); // FFT wave displacement map in VS
//Texture2D	imag_tex	: register(t2); // FFT wave displacement map in VS
...
float4 UpdateDisplacementPS(VS_QUAD_OUTPUT In) : SV_Target
{
	...
	float4 temp = real_tex.Sample(PointSampler, In.TexCoord);
	float dx = temp.y * sign_correction * g_ChoppyScale;// g_InputDxyz[addr + g_DxAddressOffset].x * sign_correction * g_ChoppyScale;
	float dy = temp.z * sign_correction * g_ChoppyScale;// g_InputDxyz[addr + g_DyAddressOffset].x * sign_correction * g_ChoppyScale;
	float dz = temp.x * sign_correction;// g_InputDxyz[addr].x * sign_correction;

	return float4(dx, dy, dz, 1);
}

// Displacement -> Normal, Folding
void GenGradientFoldingPS(VS_QUAD_OUTPUT In, out float4 gradientout : SV_Target0,
	out float4 normalout: SV_Target1)
{
